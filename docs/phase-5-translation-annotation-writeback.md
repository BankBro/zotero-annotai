...
- `写入批注` 只在翻译成功后显示。
- 点击 `写入批注` 后，当前翻译结果会覆盖写入 Zotero annotation comment。
- 如果当前选区已有可编辑 annotation，优先更新该 annotation comment。
...
9. 写入失败时 UI 显示可理解错误。
10. Zotero 原生按钮、Translate for Zotero 和 AnnotAI 三个按钮都正常。
11. 翻译浮窗拖动、缩放、关闭、置顶正常。
12. 同选区重复点击、新选区替换旧请求和 stale request 丢弃逻辑保持不变。
13. PDF 鼠标滚轮正常。
