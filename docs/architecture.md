...
当前 0.1.19 实现范围：`翻译` 浮窗只在用户点击 `写入批注` 后写入 Zotero annotation comment；只有当 Zotero selection 事件提供已有 annotation 的 `id/key` 时才更新该批注。重新拖选相同文本范围通常是新的 draft，当前实现不会按 position 自动匹配旧批注。
