...
当前阶段五已在 Provider 请求层壳上接入 `翻译` 和 `解释` 浮窗：单个默认 OpenAI-compatible Provider、设置页、连接测试、请求超时/取消和错误分类继续保留，`翻译` 和 `解释` 浮窗会调用请求层并显示真实输出。`问答` 浮窗暂不调用这个请求层。
...
临时问答：解释上下文 + 当前 reader 的问答浮窗 thread history
...
当前 0.1.20 实现范围：`翻译` 浮窗只在用户点击 `写入批注` 后写入 Zotero annotation comment；只有当 Zotero selection 事件提供已有 annotation 的 `id/key` 时才更新该批注。`解释` 浮窗已接入 Provider 输出，但不写批注、不自动高亮。重新拖选相同文本范围通常是新的 draft，当前实现不会按 position 自动匹配旧批注。
...
- 尽量避免遮挡选中文本。
- 打开对应悬浮小窗。
- 追加到 Zotero 原生文本选区弹窗内部，不替换 Zotero 自带按钮或其他插件入口。
- 不修改 Zotero 原生选区弹窗或其祖先节点的层级样式，避免破坏鼠标和滚轮事件。
...
- 错误状态。
- 停止和重试控制。
- 翻译、解释和问答重复点击时复用同一浮窗，更新当前选区上下文，并保留浮窗位置和大小。
- 打开、升级和聚焦反馈通过 reader 内底部中间的 toast 层展示，不写入选区工具条。
- 点击浮窗任意可见区域都应将该浮窗置顶。
- 浮窗只管理自身层级，处于 Zotero 阅读器普通内容、侧栏和原生选区弹窗之上，不强行改写 Zotero 原生弹窗层级。
